...
class BridgeTerminalView : IObserver{
    ...
    private:
        ...
        std::string getNSHand(boost::json::object gamestate, const std::string& dir) const;

        std::string getCardsOfSuitString(boost::json::object board, const std::string& dir, const std::string& suit) const;
        std::vector<std::string> extractAllRanksOfSuit(boost::json::array& hand, const std::string& suit) const; 
};
