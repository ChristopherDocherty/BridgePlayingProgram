...
TEST_CASE("BridgeMCTreeNodeTests -- DefaultConstructorTests") {

  using bNodeUniquePtr = std::unique_ptr<Bridge::BridgeMCTreeNode>;

  ASSERT_NO_THROW(bNodeUniquePtr baseNode =
                      std::make_unique<Bridge::BridgeMCTreeNode>());
...
TEST(BridgeMCTreeNodeTests, ParentPointerConstructorTests) {
  ...
  ASSERT_NO_THROW(
      bNodeUniquePtr childNode =
          std::make_unique<Bridge::BridgeMCTreeNode>(baseNode.get()));
